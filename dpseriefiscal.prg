...
FUNCTION DPSERIEFISCAL(nOption,cLetra)
  ...
  IF oSERIEFISCAL:nOption=1 // Incluir en caso de ser Incremental
     ...
     oSERIEFISCAL:SFI_NUMERO:=STRZERO(1,8) // 10 digitos requiere la letra
     // AutoIncremental 
  ELSE
...
//    ENDIF
  ...
  IF (!"NO-FISCAL"$oSERIEFISCAL:SFI_IMPFIS) .AND. oSERIEFISCAL:SFI_MAXZET=0 .AND. !EMPTY(oSERIEFISCAL:SFI_NUMERO)
     oSERIEFISCAL:oSFI_MAXZET:MsgErr("Requiere Capacidad","Validación")
     lResp:=.F.
  ENDIF
...
FUNCTION POSTSAVE()
  ...
  EJECUTAR("dptipdocclinumtipdoc",NIL,oSERIEFISCAL:SFI_LETRA)
  ...
  */
  ...
  IF "LIBRE"$oSERIEFISCAL:SFI_IMPFIS
     EJECUTAR("DPSERIEFISCAL_NUMREG",oSERIEFISCAL:SFI_CODSUC,oSERIEFISCAL:SFI_LETRA)
  ENDIF
  ...
  EJECUTAR("BRSELSERFISXTIP",NIL,NIL,NIL,NIL,NIL,NIL,oSERIEFISCAL:SFI_LETRA)
...
/*
  IF "Ning"$oSERIEFISCAL:SFI_IMPFIS .OR. "Caja"$oSERIEFISCAL:SFI_IMPFIS .OR. "NO-FISC"$oSERIEFISCAL:SFI_IMPFIS 
    oSERIEFISCAL:lImpFis:=.F.
  ENDIF
*/
...
/*
  IF "SMART"$oSERIEFISCAL:SFI_IMPFIS .AND. !"EVAL"$oSERIEFISCAL:SFI_IMPFIS
    cToken:="Pegar el Token oficial"
    oSERIEFISCAL:oSFI_URL:VarPut(PADR("https://bck.factura-smart.com",250),.T.) 
